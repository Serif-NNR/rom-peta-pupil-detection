...
class NestedUnetUp(nn.Module):
    ...
    def forward(self, x, shp):
        x = self.up(x)
        diffY = shp[2] - x.size()[2]
        diffX = shp[3] - x.size()[3]
...
class NestedUNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=1, deep_supervision=False, **kwargs):
        ...
        nb_filter = [32, 64, 128, 256, 512]
        self.name = "torch_UNetPP_"
        self.deep_supervision = deep_supervision

        self.pool = nn.MaxPool2d(2, 2)
        self.up = NestedUnetUp() #nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
    ...
    def forward(self, input):
        ...
        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_1 = self.conv3_1(torch.cat([x3_0, self.up(x4_0, x3_0.shape)], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, self.up(x3_1, x2_0.shape)], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, x1_2, self.up(x2_2, x1_0.shape)], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, x0_3, self.up(x1_3, x0_0.shape)], 1))
...
class NestedUNetS(nn.Module):
    def __init__(self, num_classes=1, input_channels=1, deep_supervision=False, **kwargs):
        super().__init__()

        nb_filter = [32, 64, 128, 256, 128]
        self.name = "torch_UNetPPS_x4_x4_"
        self.deep_supervision = deep_supervision
