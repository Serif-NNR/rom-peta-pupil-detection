...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = x0[:, :, 0:x1.size()[-2], 0:x1.size()[-1]]

        x2 = x[:, :, 0::2, 1::2]  # B C H/2 W/2
        x3 = x[:, :, 1::2, 1::2]  # B C H/2 W/2
        x2 = x2[:, :, 0:x3.size()[-2], 0:x3.size()[-1]]
...
class CIT(nn.Module):
    def __init__(self, img_sizeX=320, img_sizeY=240, patch_size=4, in_chans=1, out_chans=1,
        ...
        self.sizeX = int(img_sizeX/(2 ** (self.num_layers + 1)))
        self.sizeY = int(img_sizeY / (2 ** (self.num_layers + 1)))
        self.size_outX = int(img_sizeX/4)
        self.size_outY = int(img_sizeY / 4)
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_sizeX, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
        ...
        if self.final_upsample == "expand_first":
            self.up = FinalPatchExpand_X4(input_resolution=(img_sizeX // patch_size, img_sizeY // patch_size),
        ...
        self.layer2 = BasicLayer(dim=self.embed_dim * 2,
                            input_resolution= (30, 40), #(28, 28),
                            depth=2,
                            num_heads=self.num_heads * 2,
                            window_size= 2,#self.window_size,  # 7
                            mlp_ratio=self.mlp_ratio,  # 4.
                            qkv_bias=self.qkv_bias,  # True
                            qk_scale=self.qk_scale,  # None
                            drop=self.drop_rate,  # 0.
        ...
        self.layer3 = BasicLayer(dim=self.embed_dim * 4,
                            input_resolution=(15, 20), #(14, 14),
                            depth=self.depth35,
                            num_heads=self.num_heads * 4,
                            window_size= 5, #self.window_size,  # 7
                            mlp_ratio=self.mlp_ratio,  # 4.
                            qkv_bias=self.qkv_bias,  # True
                            qk_scale=self.qk_scale,  # None
                            drop=self.drop_rate,  # 0.
        ...
        self.layer4 = BasicLayer(dim=self.embed_dim * 8,
                            input_resolution=(7, 10), #(7, 7),
                            depth=2,
                            num_heads=self.num_heads * 8,
                            window_size=1, #self.window_size,  # 7
                            mlp_ratio=self.mlp_ratio,  # 4.
                            qkv_bias=self.qkv_bias,  # True
                            qk_scale=self.qk_scale,  # None
                            drop=self.drop_rate,  # 0.
        ...
        self.norm = norm_layer([self.num_features, self.sizeY, self.sizeX])

        self.Patch_Expand1 = PatchExpand(input_resolution=(7, 10),
        ...
        self.norm_up = norm_layer([self.embed_dim, self.size_outX, self.size_outY])
    ...
    def forward(self, x):   # 1,3,224,224
        ...
        Cnn = self.Up4d(Cnn)
        diffY = Mid3.size()[2] - Cnn.size()[2]
        diffX = Mid3.size()[3] - Cnn.size()[3]

        Cnn = F.pad(Cnn, [diffX // 2, diffX - diffX // 2,
                            diffY // 2, diffY - diffY // 2])
        Cnn = torch.cat((Cnn, Mid3), dim=1)
        Cnn = self.Up_conv4d(Cnn)
        Swin = self.Patch_Expand1(Swin)
        diffY = Mid3.size()[2] - Swin.size()[2]
        diffX = Mid3.size()[3] - Swin.size()[3]

        Swin = F.pad(Swin, [diffX // 2, diffX - diffX // 2,
                          diffY // 2, diffY - diffY // 2])
