...
class UniverSeg(nn.Module):
    ...
    def forward(self, target_image, support_images=None, support_labels=None, dev="cpu"):
        if support_images == None: support_images, support_labels = \
            torch.stack(target_image).to(dev), torch.stack(target_image).to(dev)
