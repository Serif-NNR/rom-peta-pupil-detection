...
def compute_iou(model, loader, threshold=0.3):
    valloss = 0
    with torch.no_grad():
        for step, (data, target, path) in enumerate(loader):
            data = data.to(device)
            target = target.to(device)

            name = path.split("\\")[-1].split(".")
            trad_conf = finetuning2[name[0]]
            temp_im = np.asarray(cv2.imread(path)[:, :, 0])
            _, start_y, end_y, start_x, end_x, smim, thx, thy = perform_traditional_method(temp_im, trad_conf)

            outputs = model(data, start_y, end_y, start_x, end_x)#["out"]
...
def train_model(model, train_loader, val_loader, loss_func, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (image, mask, path) in enumerate(tqdm(train_loader)):
            ...
            outputs = model(image, start_y, end_y, start_x, end_x)#["out"]
