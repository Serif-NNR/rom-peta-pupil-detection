...
def perform_peta(map, ellipse):
    ...
    fps_rate = 12
    if len(list_entropy) > fps_rate:
        last_center, last_size, temp_e, temp_i = list_ellipse[-1][0], list_ellipse[-1][1], list_entropy[-fps_rate: -1], list_intensity[-fps_rate:-1]
    ...
    return ellipse
list_start_x, list_start_y = [], []
def perform_peta_based_traditional(map, ellipse, norm_center, start_x, start_y):
    global list_entropy, list_intensity, list_ellipse
    corrected = False
    try:entropy, intensity = get_entropy_and_intensity(map, ellipse[0], ellipse[1], ellipse[2])
    except: return ellipse, corrected, start_x, start_y
    ...
    list_entropy.append(entropy)
    list_intensity.append(intensity)
    if corrected: start_x, start_y = list_start_x[-1], list_start_y[-1]
    else:
        list_start_x.append(start_x)
        list_start_y.append(start_y)
    return ellipse, corrected, start_x, start_y
...
for ss in dikablish_ss:
    ...
    for dsi in new_ds:
        ...
        ## PETA
        if False:
            map_ellipse, peta_corrected, start_x, start_y = perform_peta_based_traditional(last_map, raw_ellipse, new_center, start_x, start_y)
            if peta_corrected:
