...
class group_aggregation_bridge(nn.Module):
    ...
    def forward(self, xh, xl, mask):
        ...
        diffY = xh[0].size()[2] - mask.size()[2]
        diffX = xh[0].size()[3] - mask.size()[3]
...
class EGEUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        diffY = t4.size()[2] - out4.size()[2]
        diffX = t4.size()[3] - out4.size()[3]

        out4 = F.pad(out4, [diffX // 2, diffX - diffX // 2,
                            diffY // 2, diffY - diffY // 2])
        ...
        diffY = t3.size()[2] - out3.size()[2]
        diffX = t3.size()[3] - out3.size()[3]

        out3 = F.pad(out3, [diffX // 2, diffX - diffX // 2,
                            diffY // 2, diffY - diffY // 2])
        ...
        diffY = t1.size()[2] - out1.size()[2]
        diffX = t1.size()[3] - out1.size()[3]
