...
class PupilNet(nn.Module):
    ...
    def predict(self, inputs, sfac=None, offsets=None):
        with torch.no_grad():
            ...
            if False:
                im = (pred.detach().cpu().numpy()[0, 0, :, :] * 255).astype(np.uint8)
                plt.imshow(im)
                plt.show()
        pred = nn.Sigmoid()(pred).detach().cpu().squeeze()
        if False:
            im = (pred.detach().cpu().numpy() * 255).astype(np.uint8)
            plt.imshow(im)
            plt.show()
        if pred.ndim==2:
            pred = pred.unsqueeze(0)
        min_val = torch.min(pred)
        th = ((torch.max(pred) - min_val) * 0.99) + min_val
        mask = pred > th
